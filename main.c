 ...
 */
unsigned char *subtraction(unsigned char *background, unsigned char *foreground, int width, int height, int channel)
{
...
unsigned char *weather_forecast(unsigned char *background, unsigned char *foreground, unsigned char *weather, int width, int height, int channel){
    unsigned char *woman = subtraction(background, foreground, width, height, channel);
    unsigned char *temp_array = uc_arrayNew_1d(width * height * channel);
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            for(int k = 0; k < channel; k++){
...
int main()
{
    ...
    char fore_path[] = "./images1/foreground.jpg";
